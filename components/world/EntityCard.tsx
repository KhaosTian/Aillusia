...
import { UserIcon, MapPinIcon, CubeIcon, SparklesIcon, TrashIcon, ChevronRightIcon } from '../Icons';
...
export const EntityCard: React.FC<EntityCardProps> = ({ entity, onUpdate, onDelete, currentT, viewMode }) => {
    ...
    const handleAliasesChange = (val: string) => {
        // Split by common delimiters: comma, slash, space
    ...
    if (viewMode === 'list') {
        return (
             <div className="bg-white dark:bg-slate-900 rounded-xl p-4 shadow-sm border border-slate-100 dark:border-white/5 hover:shadow-md transition-shadow flex items-center gap-4 group">
                ...
                <div className="flex-1 min-w-0">
                 ...
                 <button 
                    ...
                    className="text-slate-300 hover:text-rose-500 p-2 rounded opacity-0 group-hover:opacity-100 transition-all select-none"
                >
    ...
    return (
        <div className="bg-white dark:bg-slate-900 rounded-xl p-6 shadow-sm border border-slate-100 dark:border-white/5 hover:shadow-md transition-shadow group relative flex flex-col h-full animate-fade-in">
            ...
            <div className="flex justify-between items-start mb-2">
                ...
                <button 
                    onClick={handleDelete}
                    className="text-slate-300 hover:text-rose-500 p-1 rounded hover:bg-rose-50 dark:hover:bg-rose-900/30 opacity-0 group-hover:opacity-100 transition-all select-none"
                    title="删除"
                >
                    <TrashIcon className="w-4 h-4" />
                </button>
